// Feature: Documents
// Barrel file for document-related components

export { default as PDFViewer } from './components/PDFViewer';
export { default as PDFToolbar } from './components/PDFToolbar';
export { default as AIAnalysisPanel } from './components/AIAnalysisPanel';
export { default as SummaryContent } from './components/SummaryContent';
export { default as EntityHighlight, EntityLegend } from './components/EntityHighlight';
export { default as KeyPointsList } from './components/KeyPointsList';

